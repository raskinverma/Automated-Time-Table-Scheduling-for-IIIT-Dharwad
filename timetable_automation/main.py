...
class Course:
    def __init__(self, row):
        ...
        self.title = str(row.get("Course_Title", self.code)).strip()  # course name/title
...
class Scheduler:
    ...
    def format_excel(self, filename):
        ...
        for sheet_name in wb.sheetnames:
            ...
            ws.cell(start_row, 4, "Course Name").border = thin_border
            ws.cell(start_row, 5, "Faculty").border = thin_border
            ws.cell(start_row, 6, "Color").border = thin_border
            for c in range(2, 7):
            ...
            for i, code in enumerate(color_map, start=1):
                ...
                ws.cell(start_row + i, 6, "").fill = PatternFill(start_color=color_map[code], end_color=color_map[code], fill_type="solid")
        ...
        wb.save(filename)
        print(f"Formatted timetable with borders, colors, legend, and auto-width saved in {filename}")
