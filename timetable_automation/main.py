...
class Scheduler:
    def __init__(self, slots_file, courses_file, rooms_file, global_room_usage):
        ...
        # runtime metadata
        self.course_room_map = {}      # mapping for non-elective courses generated during allocation
        self.global_room_usage = global_room_usage  # shared across departments (rooms used per day->slot)
        self.scheduled_entries = []    # records of placed sessions: {'sheet','day','slot','code','display','faculty','room'}
        self.electives_by_sheet = {}   # sheet_name -> [Course,...] (electives from CSV for that sheet)
        self.elective_room_assignment = {}  # sheet_name -> { key (code||title) : room }
    ...
    def _allocate_session(
        ...
        """
        Place a continuous block for duration_hours on the timetable.
        Records placement to self.scheduled_entries. For non-electives picks a real room and marks
        self.global_room_usage so collisions are avoided. For electives we keep timetable unchanged
        semantics (display may be 'Elective') and do not reserve rooms in global_room_usage here.
        """
        ...
        for block in free_blocks:
            ...
            if total >= duration_hours:
                ...
                # choose room for non-elective sessions only (this is the "real" allocation)
                if not is_elective:
                    ...
                    # reserve in global usage
                    for s in slots_to_use:
                        self.global_room_usage.setdefault(day, {}).setdefault(s, []).append(room)
                else:
                    # do NOT set room or reserve time — keep timetable semantics unchanged
                    room = ""

                # write to timetable and record scheduled entry
                for i, s in enumerate(slots_to_use):
                    ...
                    # keep small-slot FREE markers if needed
    ...
    def generate_timetable(self, courses_to_allocate, writer, sheet_name):
        ...
        # keep elective Course objects for legend later
        self.electives_by_sheet[sheet_name] = electives

        # place a generic Elective on the timetable (keeps timetable generation unchanged)
        if electives:
    ...
    def _compute_elective_room_assignments_legally(self, sheet_name):
        """
        For legend only: assign a classroom per elective such that:
        - No assigned elective-room collides with existing room usage in those Elective slots.
        - Each elective gets a room (prefer distinct rooms). If insufficient distinct rooms, reuse as necessary.
        This DOES NOT change global_room_usage or the timetable.
        """
        ...
        # 1) Gather all slots (day,slot) where generic 'Elective' was placed in this sheet
        ...
        # 2) For each candidate room, check if it's free across all elective_slots (based on global_room_usage)
        ...
        # 3) Prefer rooms that are already free for all elective_slots
        free_rooms = [r for r, ok in room_free_all_slots.items() if ok]
        # If none free across all slots, allow rooms that are free in at least one slot and then we will still pick them (best-effort)
        if not free_rooms:
            # compute room free counts
            room_free_counts = {r: 0 for r in candidate_rooms}
            for r in candidate_rooms:
                for day, slot in elective_slots:
                    if r not in self.global_room_usage.get(day, {}).get(slot, []):
                        room_free_counts[r] += 1
            # sort by descending free counts
            ordered = sorted(candidate_rooms, key=lambda x: (-room_free_counts[x], x))
            free_rooms = ordered

        # 4) Assign rooms round-robin from free_rooms, preferring distinct rooms
        assigned = {}
        used = set()
        idx = 0
        for e in electives:
            # try to pick a room not used yet and free across all slots
            chosen = None
            for r in free_rooms:
                if r in used:
                    continue
                # verify free across all elective_slots again (safe-check)
                ok = True
                for day, slot in elective_slots:
                    if r in self.global_room_usage.get(day, {}).get(slot, []):
                        ok = False
                ...
                if ok:
                    ...
                    break
            if not chosen:
                # fallback: pick next from free_rooms even if used
    ...
    def format_student_timetable_with_legend(self, filename):
        ...
        # Compute elective assignments per sheet BEFORE we build legends
        for sheet_name in wb.sheetnames:
            # need to ensure we compute using current global_room_usage and scheduled_entries
            self._compute_elective_room_assignments_legally(sheet_name)

        for sheet_name in wb.sheetnames:
            ws = wb[sheet_name]

            # color/merge timetable cells and build color_map
            ...
            # electives (list each elective course with its assigned classroom from elective_room_assignment)
            electives = self.electives_by_sheet.get(sheet_name, [])
    ...
    def _generate_faculty_workbook(self, faculty_filename):
        ...
        # collect faculty names from course CSV and scheduled_entries
        faculty_set = set()
        ...
        # populate from scheduled_entries; ent['display'] contains code and room when allocated
        for ent in self.scheduled_entries:
            day = ent["day"]
            slot = ent["slot"]
            display = ent["display"]  # e.g., "CS261 (R101)" or "Elective"
            # determine faculties for this scheduled entry
            if ent.get("faculty"):
        ...
        # write combined faculty workbook
        ...
        # format faculty workbook: borders, align, auto-width, colors, and merges
        wb = load_workbook(faculty_filename)
        thin = Border(left=Side(style="thin"), right=Side(style="thin"), top=Side(style="thin"), bottom=Side(style="thin"))

        # define a color palette and map per unique code
        palette = ["FFC7CE", "C6EFCE", "FFEB9C", "BDD7EE", "D9EAD3", "F4CCCC", "D9D2E9", "FCE5CD", "C9DAF8", "EAD1DC"]
        color_map = {}
        color_index = 0
            ...
            for row in range(2, ws.max_row + 1):
                ...
                while start_col <= ws.max_column:
                    ...
                    if cell.value and str(cell.value).strip() not in ["FREE", ""]:
                        ...
                        # merge same course horizontally
                        merge_count = 0
                        for col in range(start_col + 1, ws.max_column + 1):
                            next_cell = ws.cell(row=row, column=col)
                            if next_cell.value == cell.value:
    ...
    def run_all_outputs(self, dept_name_prefix="CSE", student_filename=None, faculty_filename="faculty_timetable.xlsx"):
        ...
        with pd.ExcelWriter(student_filename, engine="openpyxl") as writer:
            # First_Half and Second_Half generation as before
        ...
        # remove default sheets if present and save
        ...
        # compute legal elective-room assignments based on final scheduled_entries and global_room_usage
        for sheet in wb.sheetnames:
            self._compute_elective_room_assignments_legally(sheet)

        # format students file (adds legend using elective_room_assignment)
        self.format_student_timetable_with_legend(student_filename)

        # generate combined faculty workbook from scheduled_entries
