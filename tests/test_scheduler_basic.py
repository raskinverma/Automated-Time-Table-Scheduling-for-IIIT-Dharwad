...
class TestSchedulerBasics(unittest.TestCase):
    def setUp(self):
        ...
        ]).to_csv("data/temp_slots.csv", index=False)

        pd.DataFrame([
            {"Course_Code": "CS101", "Course_Title": "Intro", "Faculty": "Prof X", "L-T-P-S-C": "1-0-0-0-1", "Semester_Half": "1", "Elective": "0"},
        ]).to_csv("data/temp_courses.csv", index=False)

        pd.DataFrame([
            {"Room_ID": "R1", "Type": "classroom"},
            {"Room_ID": "R2", "Type": "lab"}
        ]).to_csv("data/temp_rooms.csv", index=False)

        self.sched = Scheduler("data/temp_slots.csv", "data/temp_courses.csv", "data/temp_rooms.csv", {})
